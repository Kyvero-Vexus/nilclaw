...
(defun show-help ()
  ...
  (format t "  help, --help     Show this help~%~%")
  ...
  (format t "Auth commands:~%")
  (format t "  login --provider NAME   Authenticate with an OAuth provider~%")
  (format t "  status                  Show current auth status~%~%")
  (format t "Examples:~%")
  (format t "  echo 'hello' | nilclaw chat~%")
  (format t "  nilclaw chat -i~%")
  (format t "  echo 'hello' | nilclaw chat --model ollama/qwen2:0.5b~%")
  (format t "  nilclaw auth login --provider openai-codex~%")
  (format t "  nilclaw start~%~%")
  (format t "Config search path:~%")
  (format t "  ~~/.nilclaw/init.lisp~%")
  (format t "  ~~/.nilclaw/config.lisp~%")
  (format t "  ~~/.config/nilclaw/init.lisp~%~%")
  (format t "Documentation: https://github.com/Kyvero-Vexus/nilclaw~%"))
...
(defun main ()
  ...
  (let* ((raw-args (uiop:command-line-arguments))
         ...
         (command (first args)))
    (cond
      ;; Handle --help and -h flags (before command check)
      ...
      ((string= command "migrate")
       (format t "Migration from OpenClaw to NilClaw~%~%")
       (format t "1. Run the migration script:~%")
       (format t "   sbcl --script ~~/projects/nilclaw/scripts/migrate-openclaw-config.lisp~%~%")
       (format t "2. This creates ~~/.nilclaw/init.lisp from your OpenClaw config~%~%")
       (format t "3. For OAuth providers, run: nilclaw auth login --provider openai-codex~%"))
      ((string= command "help")
       (show-help))
      (t
       (format *error-output* "[nilclaw] Unknown command: ~A~%~%" command)
       (show-help)
       (uiop:quit 1)))))
