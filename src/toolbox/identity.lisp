...
(defun build-system-prompt (&key workspace-dir model extra-context
  ...
  "Build a complete system prompt with identity, tools, and context.
   TOOLS is a list of tool-definition structs to describe to the agent.
   TOOL-FORMAT is the tool call format to instruct (:xml or :native)."
  (declare (type (or null string) workspace-dir model extra-context)
           (type list tools)
